...
class DisplayHelper:
    ...
    def prepare_dataframe(
        ...
        Args:
            sort_by: Column name to sort by ('Max H-Index', 'Citation Count', etc.)
        ...
        if sort_by and sort_by in self.df.columns:
            logger.info("Sorting by %s (%s)",
                        sort_by, 'ascending' if ascending else 'descending')

            # Convert sorting column to numeric if it's H-Index, Citation Count, or Year
            if sort_by in ['Max H-Index', 'Citation Count', 'Year']:
                # Replace 'N/A' with None for proper sorting
                self.df[sort_by] = pd.to_numeric(
                    self.df[sort_by].replace('N/A', None),
                    errors='coerce'
                )
    ...
    def format_summary(
        ...
        displayed_papers = len(self.df) if self.df is not None else total_papers

        summary = f"{displayed_papers} papers found."

        if sort_by:
            summary += f" Sorted by {sort_by} (descending)."
